  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        className={cn(
          "absolute rounded bg-primary p-1 text-background",
          direction === "rtl" ? "left-1 top-1" : "right-1 top-[0.145em]"
        )}
        onClick={() => removeFileFromSet(index)}
      >
        <span className="sr-only">remove item {index}</span>
        <BiTrash className="hover:stroke-destructive h-3 w-3 duration-200 ease-in-out" />
      </button>
    </div>
  );
});
